...
- Hosting: **Vercel** (application) + **NestJS + Better Auth + MongoDB Atlas** (auth and database)
...
| RBAC | Role-based access control enforced by NestJS guards |
| TTS | Text-to-speech |
...
- Planned stack documentation: Next.js, MongoDB Atlas, Vercel
...
[Student device] → [ReanMate on Vercel]
                        ├─ NestJS / Better Auth / MongoDB Atlas
...
| Data/Auth | MongoDB Atlas project |
...
| ID | Requirement |
|----|-------------|
| NFR-SEC-01 | Authentication shall use Better Auth (email/password) through NestJS, backed by MongoDB Atlas. |
| NFR-SEC-02 | Authorization shall enforce role separation (student vs admin) via NestJS guards and server-validated sessions. |
...
| Interface | Purpose |
|-----------|---------|
| Better Auth / NestJS | Registration and session management |
| MongoDB Atlas | Persistent authentication data; application data planned |
...
9. **Hosting:** App deployed on Vercel with MongoDB Atlas backend for the demo.
