...
export default async function HomePage() {
  const [mathChapters, historyChapters] = await Promise.all([
    getApprovedChapters(10, "math"),
    getApprovedChapters(10, "history"),
  ]);
  const mathChapter = mathChapters[0];
  const historyChapter = historyChapters[0];
    ...
    <div className="flex flex-col gap-10">
      ...
      <section>
        ...
        <div className="stagger mt-6 grid gap-4 sm:grid-cols-3 sm:gap-5">
          {GRADES.map((grade) => {
            const hasDemo = grade === 10 && (mathChapters.length > 0 || historyChapters.length > 0);
            return (
              <Link
                key={grade}
                href={`/learn/${grade}`}
